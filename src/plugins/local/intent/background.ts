...
// other plugin's `offer` sees them the way it sees a term any plugin minted;
// nothing here knows which plugin will take one.
export default defineBackground({
  terms: ({ text }) => tagText(text).map((t) => t.term),
});
