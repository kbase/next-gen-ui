...
// What the host learns about a plugin before loading any of its code. The
// author writes `plugin.config.ts`; the build adds `sdkVersion` and
// `modules` and serves the result as manifest.json, which is the one thing
// crossing this boundary that arrives as JSON over HTTP.
