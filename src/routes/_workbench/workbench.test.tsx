...
import { paneId } from '../../workbench/core';
...
describe('workbench deep links', () => {
  ...
  it('writes the URL when a page opens or gains focus, and clears it on close', async () => {
    ...
    await user.click(await within(pane).findByRole('button', { name: /assemble reads/i }));
    ...
    await user.click(await within(pane).findByRole('button', { name: /nifh search/i }));
